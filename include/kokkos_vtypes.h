 ...
 public:
  KokkosCBFineVSpinor(const LatticeInfo& info, IndexType cb)
    ...
    if (_g_info.GetNumColors() != 3) {
      MasterLog(ERROR,
                "KokkosCBFineSpinor has to have 3 colors in info. Info has %d",
                _g_info.GetNumColors());
    }
    if (_g_info.GetNumSpins() != _num_spins) {
      MasterLog(ERROR,
                "KokkosCBFineSpinor has to have %d spins in info. Info has %d",
    ...
    for (int mu = 0; mu < 4; ++mu) {
      if (l_dims[mu] % VNDims[mu] == 0) {
        ...
        MasterLog(
            ERROR,
            "Local dimension %d (=%d) not divisible by VNode::dims[%d]=%d", mu,
            l_dims[mu], mu, VNDims[mu]);
      }
    }

    _info = std::make_shared<LatticeInfo>(l_orig, l_dims, _g_info.GetNumSpins(),
                                          _g_info.GetNumColors(),
                                          _g_info.GetNodeInfo());
  ...
  inline const LatticeInfo& GetGlobalInfo() const { return _g_info; }

  inline IndexType GetCB() const { return _cb; }
  ...
  const DataType& GetData() const { return _cb_data; }

  KOKKOS_INLINE_FUNCTION
  DataType& GetData() { return _cb_data; }
...
class KokkosFineVGaugeField {
 ...
 public:
  KokkosFineVGaugeField(const LatticeInfo& info)
      : _info(info), _gauge_data_even(info, EVEN), _gauge_data_odd(info, ODD) {}
...
class KokkosCBFineVGaugeFieldDoubleCopy {
 public:
  ...
  void import(const KokkosCBFineVGaugeField<T, VN>& src_cb,
    ...
    MDPolicy policy({0, 0, 0, 0}, {cb_latdims[0], cb_latdims[1], cb_latdims[2],
                                   cb_latdims[3]});
    ...
    Kokkos::parallel_for(
        policy,
        KOKKOS_LAMBDA(const int xcb, const int y, const int z, const int t) {
      ...
      // T_minus
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(
                  _cb_data(site, 0, col, col2),
                  VN::permute(mask, othercb_data_in(n_idx, 3, col, col2)));
            }
          }
      ...
      // Z_minus
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(
                  _cb_data(site, 1, col, col2),
                  VN::permute(mask, othercb_data_in(n_idx, 2, col, col2)));
            }
          }
      ...
      // Y_minus
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(
                  _cb_data(site, 2, col, col2),
                  VN::permute(mask, othercb_data_in(n_idx, 1, col, col2)));
            }
          }
      ...
      // X_minus
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(
                  _cb_data(site, 3, col, col2),
                  VN::permute(mask, othercb_data_in(n_idx, 0, col, col2)));
            }
          }

          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(_cb_data(site, 4, col, col2),
                          cb_data_in(site, 0, col, col2));
            }
          }
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(_cb_data(site, 5, col, col2),
                          cb_data_in(site, 1, col, col2));
            }
          }
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(_cb_data(site, 6, col, col2),
                          cb_data_in(site, 2, col, col2));
            }
          }
          ...
          for (int col = 0; col < 3; ++col) {
            for (int col2 = 0; col2 < 3; ++col2) {
              ComplexCopy(_cb_data(site, 7, col, col2),
                          cb_data_in(site, 3, col, col2));
            }
          }
        });
  }
  ...
  Kokkos::parallel_for(
    ...
    // T_minus
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(
                cb_data_out(site, 0, col, col2),
                VN::permute(mask, othercb_data_in(n_idx, 3, col, col2)));
            // cb_data_out(site,0,col,col2) = VN::permute(mask,
            // othercb_data_in(n_idx,3,col,col2));
          }
        }
    ...
    // Z_minus
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(
                cb_data_out(site, 1, col, col2),
                VN::permute(mask, othercb_data_in(n_idx, 2, col, col2)));
            // cb_data_out(site,1,col,col2) = VN::permute(mask,
            // othercb_data_in(n_idx,2,col,col2));
          }
        }
    ...
    // Y_minus
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(
                cb_data_out(site, 2, col, col2),
                VN::permute(mask, othercb_data_in(n_idx, 1, col, col2)));
            // cb_data_out(site,2,col,col2) = VN::permute(mask,
            // othercb_data_in(n_idx,1,col,col2));
          }
        }
    ...
    // X_minus
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(
                cb_data_out(site, 3, col, col2),
                VN::permute(mask, othercb_data_in(n_idx, 0, col, col2)));
            // cb_data_out(site,3,col,col2) = VN::permute(mask,
            // othercb_data_in(n_idx,0,col,col2));
          }
        }

        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(cb_data_out(site, 4, col, col2),
                        cb_data_in(site, 0, col, col2));
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(cb_data_out(site, 5, col, col2),
                        cb_data_in(site, 1, col, col2));
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(cb_data_out(site, 6, col, col2),
                        cb_data_in(site, 2, col, col2));
        ...
        for (int col = 0; col < 3; ++col) {
          for (int col2 = 0; col2 < 3; ++col2) {
            ComplexCopy(cb_data_out(site, 7, col, col2),
                        cb_data_in(site, 3, col, col2));
