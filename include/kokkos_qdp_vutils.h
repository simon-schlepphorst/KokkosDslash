...
void QDPLatticeFermionToKokkosCBVSpinor(
    const LF& qdp_in, KokkosCBFineVSpinor<MGComplex<T>, VN, 4>& kokkos_out) {
  ...
  if (num_sites * VN::VecLen != num_gsites) {
    MasterLog(ERROR,
              "%s Veclen of Vector type x num_coarse_sites != num_fine_sites",
              __FUNCTION__);
  }
        ...
        for (int color = 0; color < 3; ++color) {
          for (int spin = 0; spin < 4; ++spin) {
            for (int lane = 0; lane < VN::VecLen; ++lane) {
              ...
              h_out(i, spin, color)(lane) = MGComplex<T>(
...
void QDPLatticeHalfFermionToKokkosCBVSpinor2(
    const HF& qdp_in, KokkosCBFineVSpinor<MGComplex<T>, VN, 2>& kokkos_out) {
  auto cb                = kokkos_out.GetCB();
  const QDP::Subset& sub = (cb == EVEN) ? QDP::rb[0] : QDP::rb[1];
...
// Single QDP++ vector
template <typename T, typename VN, typename LF>
void KokkosCBVSpinorToQDPLatticeFermion(
    const KokkosCBFineVSpinor<MGComplex<T>, VN, 4>& kokkos_in, LF& qdp_out) {
  ...
  if (num_csites * VN::VecLen != num_gsites) {
    MasterLog(ERROR, "%s: num_csites * veclen != num_gsites", __FUNCTION__);
  }
  ...
  Kokkos::parallel_for(
      Kokkos::RangePolicy<HostExec>(0, num_csites),
      [&](int i) {
        IndexArray c_coords;
          ...
          for (int spin = 0; spin < 4; ++spin) {
            for (int lane = 0; lane < VN::VecLen; ++lane) {
              ...
              qdp_out.elem(qdp_index).elem(spin).elem(color).real() =
                  (h_in(i, spin, color))(lane).real();
              qdp_out.elem(qdp_index).elem(spin).elem(color).imag() =
                  (h_in(i, spin, color))(lane).imag();
            }  // lane
          }    // spin
        }      // color
      }        // kokkos lambda
...
// Single QDP++ vector
template <typename T, typename VN, typename HF>
void KokkosCBVSpinor2ToQDPLatticeHalfFermion(
    const KokkosCBFineVSpinor<MGComplex<T>, VN, 2>& kokkos_in, HF& qdp_out) {
  auto cb                = kokkos_in.GetCB();
  const QDP::Subset& sub = (cb == EVEN) ? QDP::rb[0] : QDP::rb[1];
...
void QDPGaugeFieldToKokkosCBVGaugeField(
    const GF& qdp_in, KokkosCBFineVGaugeField<MGComplex<T>, VN>& kokkos_out) {
  auto cb                = kokkos_out.GetCB();
  const QDP::Subset& sub = (cb == EVEN) ? QDP::rb[0] : QDP::rb[1];
  ...
  if (sub.numSiteTable() != num_gsites) {
    MasterLog(ERROR,
              ...
              "than the KokkosCBFineVGaugeField",
              __FUNCTION__);
  }
          ...
          for (int color = 0; color < 3; ++color) {
            for (int color2 = 0; color2 < 3; ++color2) {
              for (int lane = 0; lane < VN::VecLen; ++lane) {
                ...
                h_out(i, dir, color, color2)(lane) =
                    MGComplex<T>(qdp_in[dir]
                                     .elem(qdp_index)
                                     .elem()
                                     .elem(color, color2)
                                     .real(),
...
template <typename T, typename VN, typename GF>
void KokkosCBVGaugeFieldToQDPGaugeField(
    const KokkosCBFineVGaugeField<MGComplex<T>, VN>& kokkos_in, GF& qdp_out) {
  auto cb = kokkos_in.GetCB();
          ...
          for (int color = 0; color < 3; ++color) {
            for (int color2 = 0; color2 < 3; ++color2) {
              ...
              for (int lane = 0; lane < VN::VecLen; ++lane) {
                ...
                int g_index   = CoordsToIndex(g_coords, g_dims);
                int qdp_index = sub.siteTable()[g_index];
                qdp_out[dir].elem(qdp_index).elem().elem(color, color2).real() =
                    (h_in(i, dir, color, color2))(lane).real();
                qdp_out[dir].elem(qdp_index).elem().elem(color, color2).imag() =
                    (h_in(i, dir, color, color2))(lane).imag();
              }  // lane
            }    // color2
          }      // color
        }        // mu
...
template <typename T, typename VN, typename GF>
void QDPGaugeFieldToKokkosVGaugeField(
    const GF& qdp_in, KokkosFineVGaugeField<T, VN>& kokkos_out) {
  QDPGaugeFieldToKokkosCBVGaugeField(qdp_in, kokkos_out(EVEN));
  QDPGaugeFieldToKokkosCBVGaugeField(qdp_in, kokkos_out(ODD));
}

template <typename T, typename VN, typename GF>
void KokkosGaugeFieldToQDPVGaugeField(
    const KokkosFineVGaugeField<T, VN>& kokkos_in, GF& qdp_out) {
