...
KOKKOS_FORCEINLINE_FUNCTION void mult_u_halfspinor(
    const VGaugeView<GT, VN>& gauge_in, const HalfSpinorSiteView<ST>& v_in,
    HalfSpinorSiteView<ST>& v_out, const int& i) {
#pragma unroll
...
#pragma unroll
      for (int col = 0; col < 3; ++col) {
        ComplexCMadd(v_out(row, spin), gauge_in(i, dir, row, col),
                     v_in(col, spin));
      }
    }
  }
...
// Permute Versions
template <typename GT, typename VN, typename ST, int dir>
KOKKOS_FORCEINLINE_FUNCTION void mult_u_halfspinor_perm(
    const VGaugeView<GT, VN>& gauge_in, const HalfSpinorSiteView<ST>& v_in,
    HalfSpinorSiteView<ST>& v_out, const int& i,
...
#pragma unroll
      for (int col = 0; col < 3; ++col) {
        ComplexCMadd(v_out(row, spin),
                     VN::permute(mask, gauge_in(i, dir, row, col)),
                     v_in(col, spin));
      }
    }
  }
...
template <typename GT, typename VN, typename ST, int dir>
KOKKOS_FORCEINLINE_FUNCTION void mult_adj_u_halfspinor(
    const VGaugeView<GT, VN>& gauge_in, const HalfSpinorSiteView<ST>& v_in,
    HalfSpinorSiteView<ST>& v_out, const int& i) {
#pragma unroll
  for (int spin = 0; spin < 2; ++spin) {
...
#pragma unroll
      for (int row = 0; row < 3; ++row) {
        ComplexConjMadd(v_out(row, spin), gauge_in(i, dir, col, row),
                        v_in(col, spin));
      }
    }
  }
...
template <typename GT, typename VN, typename ST, int dir>
KOKKOS_FORCEINLINE_FUNCTION void mult_adj_u_halfspinor_perm(
    const VGaugeView<GT, VN>& gauge_in, const HalfSpinorSiteView<ST>& v_in,
    HalfSpinorSiteView<ST>& v_out, const int& i,
    const typename VN::MaskType& mask) {
#pragma unroll
...
#pragma unroll
      for (int row = 0; row < 3; ++row) {
        ComplexConjMadd(v_out(row, spin),
                        VN::permute(mask, gauge_in(i, dir, col, row)),
                        v_in(col, spin));
      }
    }
  }
...
void KokkosMVLattice(const KokkosCBFineVGaugeField<GT, VN>& u_in,
                     const KokkosCBFineVSpinor<ST, VN, 2>& hspinor_in, int dir,
                     const KokkosCBFineVSpinor<ST, VN, 2>& hspinor_out,
...
void KokkosHVLattice(const KokkosCBFineGaugeField<GT>& u_in,
                     const KokkosCBFineSpinor<ST, 2>& hspinor_in, int dir,
