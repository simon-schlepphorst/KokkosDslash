...
void dslash_a(T& chi, const multi1d<U>& u, const T& psi, int isign, int cb) {
  /*     F
  ...
  /* Why are these lines split? An array syntax would help, but the problem is
   * deeper. The expression templates require NO variable args (like int's) to a
   * function and all args must be known at compile time. Hence, the function
   * names carry (as functions usually do) the meaning (and implicit args) to a
   * function.
   */
...
void dslash(LatticeFermionF& chi, const multi1d<LatticeColorMatrixF>& u,
  ...
  dslash_a<LatticeFermionF, LatticeHalfFermionF, LatticeColorMatrixF>(
      chi, u, psi, isign, cb);
}

void dslash(LatticeFermionD& chi, const multi1d<LatticeColorMatrixD>& u,
            const LatticeFermionD& psi, int isign, int cb) {
  dslash_a<LatticeFermionD, LatticeHalfFermionD, LatticeColorMatrixD>(
      chi, u, psi, isign, cb);
}
