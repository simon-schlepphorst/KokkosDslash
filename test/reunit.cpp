...
void reunit(LatticeColorMatrixF& xa, LatticeBoolean& bad, int& numbad,
            enum Reunitarize ruflag);

void reunit(LatticeColorMatrixD& xa, LatticeBoolean& bad, int& numbad,
            enum Reunitarize ruflag);
...
void reunit(LatticeColorMatrixF& xa, LatticeBoolean& bad, int& numbad,
  ...
  RealF fuzz =
      1.0e-5;  // some kind of small floating point number, should be prec. dep.

  // Extract initial components
  for (int i = 0; i < Nc; ++i)
    for (int j = 0; j < Nc; ++j) a[i][j] = peekColor(xa, i, j);
  ...
  switch (Nc) {
    ...
    case 1:
      ...
      if (ruflag == REUNITARIZE_ERROR || ruflag == REUNITARIZE_LABEL) {
      ...
      switch (ruflag) {
        case REUNITARIZE_ERROR:
          ...
          if (numbad > 0) QDP_error_exit("Unitarity violated", numbad);
          break;
    ...
    case 2:
       ...
       */
      if (ruflag == REUNITARIZE_ERROR || ruflag == REUNITARIZE_LABEL) {
        for (int c = 0; c < Nc; ++c) row[c] = a[c][Nc - 1];
      }

      /* normalise the first row */
      /* t1 = sqrt(u^t . u) */
      t1 = localNorm2(a[0][0]);
      for (int c = 1; c < Nc; ++c) t1 += localNorm2(a[c][0]);
      t1 = sqrt(t1);

      /* overwrite the first row with the rescaled value */
      /* u <- u/t1 */
      t4 = 1 / t1;
      for (int c = 0; c < Nc; ++c) a[c][0] *= t4;
    ...
    case 3:
      ...
      /* t2 <- u^t.v */
      t2 = adj(a[0][0]) * a[0][1];
      for (int c = 1; c < Nc; ++c) t2 += adj(a[c][0]) * a[c][1];

      /* orthogonalize the second row relative to the first row */
      /* v <- v - t2*u */
      for (int c = 0; c < Nc; ++c) a[c][1] -= t2 * a[c][0];

      /* normalise the second row */
      /* t3 = sqrt(u^t . u) */
      t3 = localNorm2(a[0][1]);
      for (int c = 1; c < Nc; ++c) t3 += localNorm2(a[c][1]);
      t3 = sqrt(t3);

      /* overwrite the second row with the rescaled value */
      /* v <- v/t3 */
      t4 = 1 / t3;
      for (int c = 0; c < Nc; ++c) a[c][1] *= t4;
    ...
    default:
      if (Nc > 3) {
        /* If you want to check unitarity, I have to save the third row
         * somewhere */
        if (ruflag == REUNITARIZE_ERROR || ruflag == REUNITARIZE_LABEL) {
          for (int c = 0; c < Nc; ++c) row[c] = a[c][Nc - 1];
        }
        ...
        for (int j = 1; j < Nc; j++) {
          ...
          /* t1 = sqrt(v^t . v) */
          t1 = localNorm2(a[0][j]);
          for (int c = 1; c < Nc; ++c) t1 += localNorm2(a[c][j]);
          t1 = sqrt(t1);

          /* overwrite the j-th row with the rescaled value */
          /* v <- v/t1 */
          t3 = 1 / t1;
          for (int c = 0; c < Nc; ++c) a[c][j] *= t3;
        ...
        for (int j = 0; j < Nc; j++)
          for (int i = 0; i < Nc; i++) b[j][i] = a[j][i];

        for (int j = 0; j < Nc; j++) {
          for (int i = 0; i <= j; i++) {
            t2 = b[j][i];
            for (int c = 0; c < i; c++) t2 -= b[c][i] * b[j][c];
          ...
          for (int i = (j + 1); i < Nc; i++) {
            t2 = b[j][i];
            for (int c = 0; c < j; c++) t2 -= b[c][i] * b[j][c];
        ...
        switch (ruflag) {
          case REUNITARIZE_ERROR:
            /* Gripe and stop if unitarity is violated. */
            numbad = toInt(sum(
                where(sigmasq > fuzz, LatticeInteger(1), LatticeInteger(0))));
            if (numbad > 0) QDP_error_exit("Unitarity violated", numbad);
            break;
          case REUNITARIZE_LABEL:
            /* Label the bad guys if unitarity is violated. */
            bad = sigmasq > fuzz;
            numbad =
                toInt(sum(where(bad, LatticeInteger(1), LatticeInteger(0))));
          default: break;
        }
      } else
  ...
  for (int i = 0; i < Nc; ++i)
    for (int j = 0; j < Nc; ++j) pokeColor(xa, a[i][j], i, j);
}
...
void reunit(LatticeColorMatrixD& xa, LatticeBoolean& bad, int& numbad,
  ...
  RealD fuzz = 1.0e-10;  // some kind of small floating point number, should be
                         // prec. dep.
