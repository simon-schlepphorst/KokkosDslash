    ...
    for (auto const& x: xyz_values) {
        for (auto const& t: t_values){
            ...
            for(
                ...
                MasterLog(INFO,
                          "sites_per_team=%d time per iter = %lf (usec) Performance: %lf GFLOPS", sites_per_team,time_taken*1.0e6/(double)(iters), flops/(time_taken*1.0e9));
                MasterLog(INFO,
                          "sites_per_team=%d Effective BW: %lf GB/sec", sites_per_team,(bytes_in+bytes_out)/(time_taken*1.0e9));
