...
TEST(TestVNode, TestVSpinor) {
  ...
  bool same_global_vectype =
      std::is_same<SpinorType::VecType,
                   SIMDComplex<float, VectorLength>>::value;
  ASSERT_EQ(same_global_vectype, true);

  bool same_thread_vectype =
      std::is_same<VN::VecType, SIMDComplex<float, VectorLength>>::value;
...
TEST(TestVNode, TestVGauge) {
  ...
  bool same_global_vectype =
      std::is_same<GaugeType::VecType, SIMDComplex<float, VectorLength>>::value;
  ASSERT_EQ(same_global_vectype, true);
...
float computeLane(const IndexArray& coords, const IndexArray& cb_latdims) {
  ...
  if (VectorLength == 4) {
    if (coords[2] < cb_latdims[2] / 2 && coords[3] < cb_latdims[3] / 2) {
      value = 0;
    }
    if (coords[2] >= cb_latdims[2] / 2 && coords[3] < cb_latdims[3] / 2) {
      value = 1;
  ...
  if (VectorLength == 8) {
    if (coords[1] < cb_latdims[1] / 2 && coords[2] < cb_latdims[2] / 2 &&
  ...
  if (VectorLength == 16) {
    if (coords[0] < cb_latdims[0] / 2 && coords[1] < cb_latdims[1] / 2 &&
        coords[2] < cb_latdims[2] / 2 && coords[3] < cb_latdims[3] / 2) {
      value = 0;
    }
...
TEST(TestVNode, TestPackSpinor) {
  ...
  MasterLog(INFO, "cb_latdims=(%d,%d,%d,%d)", cb_latdims[0], cb_latdims[1],
            cb_latdims[2], cb_latdims[3]);
  Kokkos::parallel_for(
      Kokkos::RangePolicy<HostExec>(0, num_cbsites), [&](int i) {
        IndexArray coords;
  ...
  bool same_global_vectype =
      std::is_same<SpinorType::VecType, SIMDComplex<float, VN::VecLen>>::value;
  ASSERT_EQ(same_global_vectype, true);

  const LatticeInfo& vinfo = kokkos_spinor.GetInfo();
  int num_vcbsites         = vinfo.GetNumCBSites();
  Kokkos::parallel_for(Kokkos::RangePolicy<HostExec>(0, num_vcbsites),
                       [=](int i) {
                         for (int color = 0; color < 3; ++color) {
                           for (int spin = 0; spin < 4; ++spin) {
  ...
  Kokkos::parallel_for(
      Kokkos::RangePolicy<HostExec>(0, num_cbsites), [=](int i) {
        for (int color = 0; color < 3; ++color) {
          for (int spin = 0; spin < 4; ++spin) {
            float ref_re = qdp_in.elem(rb[EVEN].siteTable()[i])
                               .elem(spin)
                               .elem(color)
                               .real();
            float ref_im = qdp_in.elem(rb[EVEN].siteTable()[i])
                               .elem(spin)
                               .elem(color)
                               .imag();

            float out_re = back_spinor.elem(rb[EVEN].siteTable()[i])
                               .elem(spin)
                               .elem(color)
                               .real();
            float out_im = back_spinor.elem(rb[EVEN].siteTable()[i])
                               .elem(spin)
                               .elem(color)
                               .imag();
  ...
  for (int cb = EVEN; cb <= ODD; ++cb) {
    Kokkos::parallel_for(
        Kokkos::RangePolicy<HostExec>(0, num_cbsites), [=](int i) {
          for (int color = 0; color < 3; ++color) {
            for (int spin = 0; spin < 4; ++spin) {
              float ref_re = qdp_in.elem(rb[cb].siteTable()[i])
                                 .elem(spin)
                                 .elem(color)
                                 .real();
              float ref_im = qdp_in.elem(rb[cb].siteTable()[i])
                                 .elem(spin)
                                 .elem(color)
                                 .imag();

              float out_re = qdp_out.elem(rb[cb].siteTable()[i])
                                 .elem(spin)
                                 .elem(color)
                                 .real();
              float out_im = qdp_out.elem(rb[cb].siteTable()[i])
                                 .elem(spin)
                                 .elem(color)
                                 .imag();
...
TEST(TestVNode, TestPackGauge) {
  ...
  bool same_global_vectype =
      std::is_same<GaugeType::VecType, SIMDComplex<float, VN::VecLen>>::value;
  ASSERT_EQ(same_global_vectype, true);
  ...
  for (int mu = 0; mu < Nd; ++mu) {
    Kokkos::parallel_for(Kokkos::RangePolicy<HostExec>(0, num_cbsites),
                         [=](int i) {
                           for (int color = 0; color < 3; ++color) {
                             for (int color2 = 0; color2 < 3; ++color2) {
                               float ref_re = u[mu]
                                                  .elem(rb[EVEN].siteTable()[i])
                                                  .elem()
                                                  .elem(color, color2)
                                                  .real();
                               float ref_im = u[mu]
                               ...
                               float out_re = u_back[mu]
                                                  .elem(rb[EVEN].siteTable()[i])
                                                  .elem()
                                                  .elem(color, color2)
                                                  .real();
                               float out_im = u_back[mu]
                                                  .elem(rb[EVEN].siteTable()[i])
                                                  .elem()
                                                  .elem(color, color2)
                                                  .imag();
    ...
    for (int cb = EVEN; cb <= ODD; ++cb) {
      Kokkos::parallel_for(Kokkos::RangePolicy<HostExec>(0, num_cbsites),
                           [=](int i) {
                             for (int color = 0; color < 3; ++color) {
                               for (int color2 = 0; color2 < 3; ++color2) {
                                 float ref_re = qdp_in[mu]
                                                    .elem(rb[cb].siteTable()[i])
                                                    .elem()
                                                    .elem(color, color2)
                                                    .real();
                                 float ref_im = qdp_in[mu]
                                                    .elem(rb[cb].siteTable()[i])
                                                    .elem()
                                                    .elem(color, color2)
                                                    .imag();

                                 float out_re = qdp_out[mu]
                                                    .elem(rb[cb].siteTable()[i])
                                                    .elem()
                                                    .elem(color, color2)
                                                    .real();
                                 float out_im = qdp_out[mu]
                                                    .elem(rb[cb].siteTable()[i])
                                                    .elem()
                                                    .elem(color, color2)
                                                    .imag();
...
TEST(TestKokkos, TestVSpinProject) {
...

  {
    ...
    KokkosVProjectLattice<MGComplex<REAL>, VN, SIMDComplex<REAL, VN::VecLen>, 0,
                          -1>(kokkos_in, kokkos_hspinor_out);
...

  {
    ...
    KokkosVProjectLattice<MGComplex<REAL>, VN, SIMDComplex<REAL, VN::VecLen>, 1,
                          -1>(kokkos_in, kokkos_hspinor_out);
    KokkosCBVSpinor2ToQDPLatticeHalfFermion(kokkos_hspinor_out, kokkos_out);
    qdp_out[rb[0]] -= kokkos_out;
...

  {
    ...
    KokkosVProjectLattice<MGComplex<REAL>, VN, SIMDComplex<REAL, VN::VecLen>, 2,
                          -1>(kokkos_in, kokkos_hspinor_out);
    KokkosCBVSpinor2ToQDPLatticeHalfFermion(kokkos_hspinor_out, kokkos_out);
    qdp_out[rb[0]] -= kokkos_out;
...

  {
    ...
    KokkosVProjectLattice<MGComplex<REAL>, VN, SIMDComplex<REAL, VN::VecLen>, 3,
                          -1>(kokkos_in, kokkos_hspinor_out);
    KokkosCBVSpinor2ToQDPLatticeHalfFermion(kokkos_hspinor_out, kokkos_out);
    qdp_out[rb[0]] -= kokkos_out;
...
TEST(TestKokkos, TestDslash) {
...
#if defined(MG_USE_CUDA) || defined(MG_USE_HIP)
  IndexArray blockings[6] = {{1, 1, 1, 1}, {2, 2, 2, 4}, {4, 4, 1, 2},
                             {4, 2, 8, 4}, {8, 4, 1, 4}, {16, 4, 1, 1}};
#else
  IndexArray blockings[6] = {{1, 1, 1, 1}, {2, 2, 2, 4}, {4, 4, 1, 2},
                             {4, 2, 8, 4}, {8, 4, 1, 4}, {4, 2, 2, 16}};
#endif
  ...
  for (int b = 0; b < 6; ++b) {
    ...
    for (int cb = 0; cb < 2; ++cb) {
      SpinorType& out_spinor =
          (cb == EVEN) ? kokkos_spinor_even : kokkos_spinor_odd;
      SpinorType& in_spinor =
          (cb == EVEN) ? kokkos_spinor_odd : kokkos_spinor_even;
	...
	<< " , " << kokkos_out.elem(rb[cb].siteTable()[site]).elem(spin).elem(color).imag() << " ) " << std::endl;
        ...
        Kokkos::parallel_for(
            Kokkos::RangePolicy<HostExec>(0, num_sites), [=](int i) {
              int qdp_idx = rb[cb].siteTable()[i];
              for (int spin = 0; spin < 4; ++spin) {
                for (int color = 0; color < 3; ++color) {
                  REAL32 re = std::abs(
                      psi_out.elem(qdp_idx).elem(spin).elem(color).real());
                  REAL32 im = std::abs(
                      psi_out.elem(qdp_idx).elem(spin).elem(color).imag());
                  ASSERT_LT(re, 3.5e-6);
                  ASSERT_LT(im, 3.5e-6);
                }
              }
