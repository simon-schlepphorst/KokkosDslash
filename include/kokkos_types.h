...
class KokkosCBFineGaugeField {
 public:
  KokkosCBFineGaugeField(const LatticeInfo& info, IndexType cb)
      : _cb_gauge_data("cb_gauge_data", info.GetNumCBSites()),
        _info(info),
        _cb(cb) {
    if (_info.GetNumColors() != 3) {
      MasterLog(ERROR,
                "KokkosFineGaugeField needs to have 3 colors. Info has %d",
                _info.GetNumColors());
    }
  }

  inline const T& operator()(int site, int dir, int color1, int color2) const {
    return _cb_gauge_data(site, dir, color1, color2);
  }

  inline T& operator()(int site, int dir, int color1, int color2) {
    return _cb_gauge_data(site, dir, color1, color2);
  }

  using DataType = Kokkos::View<T* [4][3][3], GaugeLayout, MemorySpace>;

  DataType& GetData() { return _cb_gauge_data; }

  const DataType& GetData() const { return _cb_gauge_data; }

  IndexType GetCB() const { return _cb; }
...
class KokkosFineGaugeField {
 ...
 public:
  KokkosFineGaugeField(const LatticeInfo& info)
...
template <typename T, int N>
using GaugeViewVec =
    typename KokkosCBFineGaugeField<MG::SIMDComplex<T, N>>::DataType;
