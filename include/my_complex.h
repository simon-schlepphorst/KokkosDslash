...
class complex {
 ...
 public:
  ...
  KOKKOS_INLINE_FUNCTION complex<T>& operator=(const complex<T1>& src) {
    _re = src._re;
    _im = src._im;
    return *this;
  }
...
class complex<float> : public float2 {
 public:
  ...
  explicit KOKKOS_INLINE_FUNCTION complex<float>(const float& re,
                                                 const float& im) {
    x = re;
    y = im;
  }
  ...
  template <typename T1>
  KOKKOS_INLINE_FUNCTION complex<float>& operator=(const complex<T1>& src) {
    x = src.x;
    y = src.y;
    return *this;
  }
  ...
  KOKKOS_INLINE_FUNCTION
  const float& real() const { return x; }

  KOKKOS_INLINE_FUNCTION
  float& imag() { return y; }

  KOKKOS_INLINE_FUNCTION
  const float& imag() const { return y; }
