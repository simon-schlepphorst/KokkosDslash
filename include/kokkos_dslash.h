...
void ComputeSiteTable(
    int _n_xh, int _n_x, int _n_y, int _n_z, int _n_t,
    Kokkos::View<int *[2][8], NeighLayout, MemorySpace> _table) {
  int num_sites = _n_xh * _n_y * _n_z * _n_t;
  Kokkos::parallel_for(
      Kokkos::RangePolicy<ExecSpace>(0, num_sites), KOKKOS_LAMBDA(int site) {
        for (int target_cb = 0; target_cb < 2; ++target_cb) {
          ...
          if (t > 0) {
            _table(site, target_cb, T_MINUS) =
                xcb + _n_xh * (y + _n_y * (z + _n_z * (t - 1)));
          } else {
            _table(site, target_cb, T_MINUS) =
                xcb + _n_xh * (y + _n_y * (z + _n_z * (_n_t - 1)));
          }

          if (z > 0) {
            _table(site, target_cb, Z_MINUS) =
                xcb + _n_xh * (y + _n_y * ((z - 1) + _n_z * t));
          } else {
            _table(site, target_cb, Z_MINUS) =
                xcb + _n_xh * (y + _n_y * ((_n_z - 1) + _n_z * t));
          }

          if (y > 0) {
            _table(site, target_cb, Y_MINUS) =
                xcb + _n_xh * ((y - 1) + _n_y * (z + _n_z * t));
          } else {
            _table(site, target_cb, Y_MINUS) =
                xcb + _n_xh * ((_n_y - 1) + _n_y * (z + _n_z * t));
          }

          if (x > 0) {
            _table(site, target_cb, X_MINUS) =
                ((x - 1) / 2) + _n_xh * (y + _n_y * (z + _n_z * t));
          } else {
            _table(site, target_cb, X_MINUS) =
                ((_n_x - 1) / 2) + _n_xh * (y + _n_y * (z + _n_z * t));
          }

          if (x < _n_x - 1) {
            _table(site, target_cb, X_PLUS) =
                ((x + 1) / 2) + _n_xh * (y + _n_y * (z + _n_z * t));
          } else {
            _table(site, target_cb, X_PLUS) =
                0 + _n_xh * (y + _n_y * (z + _n_z * t));
          }

          if (y < _n_y - 1) {
            _table(site, target_cb, Y_PLUS) =
                xcb + _n_xh * ((y + 1) + _n_y * (z + _n_z * t));
          } else {
            _table(site, target_cb, Y_PLUS) =
                xcb + _n_xh * (0 + _n_y * (z + _n_z * t));
          }

          if (z < _n_z - 1) {
            _table(site, target_cb, Z_PLUS) =
                xcb + _n_xh * (y + _n_y * ((z + 1) + _n_z * t));
          } else {
            _table(site, target_cb, Z_PLUS) =
                xcb + _n_xh * (y + _n_y * (0 + _n_z * t));
          }

          if (t < _n_t - 1) {
            _table(site, target_cb, T_PLUS) =
                xcb + _n_xh * (y + _n_y * (z + _n_z * (t + 1)));
          } else {
            _table(site, target_cb, T_PLUS) =
                xcb + _n_xh * (y + _n_y * (z + _n_z * (0)));
          }
        }  // target CB
      });
}
...
class SiteTable {
 public:
  SiteTable(int n_xh, int n_y, int n_z, int n_t)
      : _n_x(2 * n_xh), _n_xh(n_xh), _n_y(n_y), _n_z(n_z), _n_t(n_t) {
#if defined(MG_KOKKOS_USE_NEIGHBOR_TABLE)
    _table = Kokkos::View<int *[2][8], NeighLayout, MemorySpace>(
        "table", n_xh * n_y * n_z * n_t);
    ComputeSiteTable(n_xh, 2 * n_xh, n_y, n_z, n_t, _table);
#endif
  }

  KOKKOS_FORCEINLINE_FUNCTION
  int coords_to_idx(const int &xcb, const int &y, const int &z,
                    const int &t) const {
  ...
  int NeighborTMinus(int site, int target_cb) const {
    ...
    return (t > 0) ? xcb + _n_xh * (y + _n_y * (z + _n_z * (t - 1)))
                   : xcb + _n_xh * (y + _n_y * (z + _n_z * (_n_t - 1)));
  }
  ...
  int NeighborZMinus(int site, int target_cb) const {
    ...
    return (z > 0) ? xcb + _n_xh * (y + _n_y * ((z - 1) + _n_z * t))
                   : xcb + _n_xh * (y + _n_y * ((_n_z - 1) + _n_z * t));
  }
  ...
  int NeighborYMinus(int site, int target_cb) const {
    ...
    return (y > 0) ? xcb + _n_xh * ((y - 1) + _n_y * (z + _n_z * t))
                   : xcb + _n_xh * ((_n_y - 1) + _n_y * (z + _n_z * t));
  }
  ...
  int NeighborXMinus(int site, int target_cb) const {
    ...
    return (x > 0) ? ((x - 1) / 2) + _n_xh * (y + _n_y * (z + _n_z * t))
                   : ((_n_x - 1) / 2) + _n_xh * (y + _n_y * (z + _n_z * t));
  }
  ...
  int NeighborXPlus(int site, int target_cb) const {
    ...
    return (x < _n_x - 1) ? ((x + 1) / 2) + _n_xh * (y + _n_y * (z + _n_z * t))
                          : 0 + _n_xh * (y + _n_y * (z + _n_z * t));
  }
  ...
  int NeighborYPlus(int site, int target_cb) const {
    ...
    return (y < _n_y - 1) ? xcb + _n_xh * ((y + 1) + _n_y * (z + _n_z * t))
                          : xcb + _n_xh * (0 + _n_y * (z + _n_z * t));
  }
  ...
  int NeighborZPlus(int site, int target_cb) const {
    ...
    return (z < _n_z - 1) ? xcb + _n_xh * (y + _n_y * ((z + 1) + _n_z * t))
                          : xcb + _n_xh * (y + _n_y * (0 + _n_z * t));
  }
  ...
  int NeighborTPlus(int site, int target_cb) const {
    ...
    return (t < _n_t - 1) ? xcb + _n_xh * (y + _n_y * (z + _n_z * (t + 1)))
                          : xcb + _n_xh * (y + _n_y * (z + _n_z * (0)));
  }
#endif
...
struct DslashFunctor {
  ...
  KOKKOS_FORCEINLINE_FUNCTION
  void operator()(const int &xcb, const int &y, const int &z,
                  const int &t) const {
  ...
  void operator()(const TeamHandle &team) const {
    ...
    const int end_idx   = start_idx + sites_per_team < num_sites
                              ? start_idx + sites_per_team
                              : num_sites;
    ...
    // T - minus
    KokkosProjectDir3<ST, TST, isign>(
        s_in, proj_res, neigh_table.NeighborTMinus(site, target_cb));
    ...
    // Z - minus
    KokkosProjectDir2<ST, TST, isign>(
        s_in, proj_res, neigh_table.NeighborZMinus(site, target_cb));
    mult_adj_u_halfspinor<GT, TST>(g_in_src_cb, proj_res, mult_proj_res,
                                   neigh_table.NeighborZMinus(site, target_cb),
                                   2);
    KokkosRecons23Dir2<TST, isign>(mult_proj_res, res_sum);

    // Y - minus
    KokkosProjectDir1<ST, TST, isign>(
        s_in, proj_res, neigh_table.NeighborYMinus(site, target_cb));
    mult_adj_u_halfspinor<GT, TST>(g_in_src_cb, proj_res, mult_proj_res,
                                   neigh_table.NeighborYMinus(site, target_cb),
                                   1);
    KokkosRecons23Dir1<TST, isign>(mult_proj_res, res_sum);

    // X - minus
    KokkosProjectDir0<ST, TST, isign>(
        s_in, proj_res, neigh_table.NeighborXMinus(site, target_cb));
    mult_adj_u_halfspinor<GT, TST>(g_in_src_cb, proj_res, mult_proj_res,
                                   neigh_table.NeighborXMinus(site, target_cb),
                                   0);
    KokkosRecons23Dir0<TST, isign>(mult_proj_res, res_sum);

    // X - plus
    KokkosProjectDir0<ST, TST, -isign>(
        s_in, proj_res, neigh_table.NeighborXPlus(site, target_cb));
    mult_u_halfspinor<GT, TST>(g_in_target_cb, proj_res, mult_proj_res, site,
                               0);
    KokkosRecons23Dir0<TST, -isign>(mult_proj_res, res_sum);

    // Y - plus
    KokkosProjectDir1<ST, TST, -isign>(
        s_in, proj_res, neigh_table.NeighborYPlus(site, target_cb));
    mult_u_halfspinor<GT, TST>(g_in_target_cb, proj_res, mult_proj_res, site,
                               1);
    KokkosRecons23Dir1<TST, -isign>(mult_proj_res, res_sum);
    ...
    // Z - plus
    KokkosProjectDir2<ST, TST, -isign>(
        s_in, proj_res, neigh_table.NeighborZPlus(site, target_cb));
    mult_u_halfspinor<GT, TST>(g_in_target_cb, proj_res, mult_proj_res, site,
                               2);
    KokkosRecons23Dir2<TST, -isign>(mult_proj_res, res_sum);
    ...
    // T - plus
    KokkosProjectDir3<ST, TST, -isign>(
        s_in, proj_res, neigh_table.NeighborTPlus(site, target_cb));
    mult_u_halfspinor<GT, TST>(g_in_target_cb, proj_res, mult_proj_res, site,
                               3);
    KokkosRecons23Dir3<TST, -isign>(mult_proj_res, res_sum);
...
class KokkosDslash {
 ...
 public:
  KokkosDslash(const LatticeInfo &info, int sites_per_team = 1)
      : _info(info),
        _neigh_table(
            info.GetCBLatticeDimensions()[0], info.GetCBLatticeDimensions()[1],
            info.GetCBLatticeDimensions()[2], info.GetCBLatticeDimensions()[3]),
  ...
  void operator()(const KokkosCBFineSpinor<ST, 4> &fine_in,
                  ...
                  KokkosCBFineSpinor<ST, 4> &fine_out, int plus_minus) const {
...
#if !defined(MG_FLAT_PARALLEL_DSLASH)
    ThreadExecPolicy policy(num_sites / _sites_per_team, Kokkos::AUTO(),
                            Veclen<ST>::value);
#endif
    if (plus_minus == 1) {
      if (target_cb == 0) {
        DslashFunctor<GT, ST, TST, 1, 0> f = {
            s_in,      g_in_src_cb,     g_in_target_cb, s_out,
  ...
  void operator()(const KokkosCBFineSpinor<ST, 4> &fine_in,
                  const KokkosFineGaugeField<GT> &gauge_in,
                  KokkosCBFineSpinor<ST, 4> &fine_out, int plus_minus,
