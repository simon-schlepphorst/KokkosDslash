...
KOKKOS_FORCEINLINE_FUNCTION void KokkosProjectDir0(
    const SpinorView<T>& in, HalfSpinorSiteView<T2>& spinor_out, int i) {
...
KOKKOS_FORCEINLINE_FUNCTION void KokkosProjectDir2(
  ...
  for (int color = 0; color < 3; ++color) {
    ...
    A_add_sign_iB(spinor_out(color, 1), in(i, color, 1), -sign,
                  in(i, color, 3));
  }
}
...
#else
  Kokkos::parallel_for(
      num_sites, KOKKOS_LAMBDA(const int i) {
#endif
...
// #ifndef MG_FLAT_PARALLEL_DSLASH
  ...
  if (dir == 0) {
    KokkosRecons23Dir0<T2, isign>(hspinor_in, res);
  } else if (dir == 1) {
    KokkosRecons23Dir1<T2, isign>(hspinor_in, res);

  } else if (dir == 2) {
    KokkosRecons23Dir2<T2, isign>(hspinor_in, res);
  } else {
    KokkosRecons23Dir3<T2, isign>(hspinor_in, res);
  }
