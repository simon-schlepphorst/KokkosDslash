...
TEST(TestVectype, VectypeCreateD4) { SIMDComplex<double, 4> v2; }
...
TEST(TestVectype, TestLaneAccessorsD4) {
  SIMDComplex<double, 4> v4;
  for (int i = 0; i < v4.len(); ++i) v4.set(i, MG::MGComplex<double>(i, -i));
...
TEST(TestVectype, VectypeCopyD4) {
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const TeamHandle& team) {
        Kokkos::parallel_for(
            Kokkos::TeamThreadRange(team, 0, 1), [=](const int site) {
              SIMDComplex<double, 4> v4;
              for (int i = 0; i < v4.len(); ++i)
...
TEST(TestVectype, VectypeLoadStore) {
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const TeamHandle& team) {
        Kokkos::parallel_for(Kokkos::TeamThreadRange(team, 0, 1),
                             [=](const int site) {
                               SIMDComplex<double, 4> v4;
                               for (int i = 0; i < v4.len(); ++i)
                                 v4.set(i, MG::MGComplex<double>(i, -i));
...
TEST(TestVectype, TestLaneAccessorsFVLEN) {
  SIMDComplex<float, VLEN> v4;
  for (int i = 0; i < v4.len(); ++i) v4.set(i, MG::MGComplex<float>(i, -i));
